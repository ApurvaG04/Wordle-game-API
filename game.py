...
async def guess(gameId):
    ...
    if word == secretWord:
        ...
        callbackURL = await db_select.fetch_one("SELECT URL FROM listen WHERE username=:username", username)
        if callbackURL != "null":
            finished = await db_select.fetch_one("SELECT finished FROM game WHERE id=:id", values={"id": gameId})
            httpx.post('https://127.0.0.1:5400/scoreupdates/reply', result=finished)
